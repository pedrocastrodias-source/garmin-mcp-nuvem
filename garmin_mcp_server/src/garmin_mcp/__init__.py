...
def init_api(email, password):
    ...
    # Decode GARMIN_TOKENS_BASE64 if provided
    ...
    if tokens_base64:
        ...
        try:
            decoded_data = base64.b64decode(tokens_base64.strip()).decode('utf-8')
            with open(token_json_path, "w", encoding="utf-8") as f:
                f.write(decoded_data)
            print(f"Decoded and wrote GARMIN_TOKENS_BASE64 to {token_json_path}", file=sys.stderr)
        except Exception as e:
            print(f"Error decoding GARMIN_TOKENS_BASE64: {e}", file=sys.stderr)
...
def main():
    ...
    if not garmin_client:
        print("AVISO: Cliente Garmin Connect nao autenticado na inicializacao. O servidor web continuara ativo.", file=sys.stderr)
    else:
    ...
    if transport == "sse":
        ...
        starlette_app.add_middleware(PreventBufferingMiddleware)
        import uvicorn
